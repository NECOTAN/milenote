...
export default function BottomNav() {
  ...
  return (
    ...
    <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-md z-50 h-16 rounded-full bg-white/90 backdrop-blur-md shadow-lg border border-slate-100">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.key}
              href={item.href}
              className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors ${isActive ? "text-slate-900" : "text-slate-400 hover:text-slate-600"
                }`}
            >
